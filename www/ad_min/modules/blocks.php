...
function BlocksAdmin() {
	...
	while (list($bid, $bkey, $title, $bposition, $weight, $active, $blockfile, $view, $expire, $action, $con1, $bposition1, $weight1, $con2, $bposition2, $weight2) = mysqli_fetch_row($result)) {
		...
		if ($bposition == "l") {
			$bposition = "<img src=\"ad_min/pic/left.gif\" border=\"0\" alt=\"Левый блок\" title=\"Левый блок\"> Слева";
		} elseif ($bposition == "r") {
			$bposition = "Справа <img src=\"ad_min/pic/right.gif\" border=\"0\" alt=\"Правый блок\" title=\"Правый блок\">";
		} elseif ($bposition == "c") {
			$bposition = "<img src=\"ad_min/pic/right.gif\" border=\"0\" alt=\"Центральный блок\" title=\"Центральный блок\">&nbsp;По центру вверху&nbsp;<img src=\"ad_min/pic/left.gif\" border=\"0\" alt=\"Центральный блок\" title=\"Центральный блок\">";
		} elseif ($bposition == "d") {
			$bposition = "<img src=\"ad_min/pic/right.gif\" border=\"0\" alt=\"Центральный блок\" title=\"Центральный блок\">&nbsp;По центру внизу&nbsp;<img src=\"ad_min/pic/left.gif\" border=\"0\" alt=\"Центральный блок\" title=\"Центральный блок\">";
		} elseif ($bposition == "b") {
			$bposition = "<img src=\"ad_min/pic/up.gif\" border=\"0\" alt=\"Баннер\" title=\"Баннер\">&nbsp;Верхний баннер&nbsp;<img src=\"ad_min/pic/up.gif\" border=\"0\" alt=\"Баннер\" title=\"Баннер\">";
		} elseif ($bposition == "f") {
			$bposition = "<img src=\"ad_min/pic/down.gif\" border=\"0\" alt=\"Баннер\" title=\"Баннер\">&nbsp;Нижний баннер&nbsp;<img src=\"ad_min/pic/down.gif\" border=\"0\" alt=\"Баннер\" title=\"Баннер\">";
		}
		echo "<td align=\"center\"><nobr>$bposition</nobr></td><td align=\"center\">$weight</td><td align=\"center\">";
		if ($con1) echo "<a href=\"".$admin_file.".php?op=BlocksOrder&weight=$weight&bidori=$bid&weightrep=$weight_minus&bidrep=$con1\"><img src=\"ad_min/pic/up.gif\" alt=\"Переместить вверх\" title=\"Переместить вверх\" border=\"0\"></a> ";
		if ($con2) echo "<a href=\"".$admin_file.".php?op=BlocksOrder&weight=$weight&bidori=$bid&weightrep=$weight_plus&bidrep=$con2\"><img src=\"ad_min/pic/down.gif\" alt=\"Переместить вниз\" title=\"Переместить вниз\" border=\"0\"></a>";
		echo"</td>";
		if ($bkey == "") {
			$type = "HTML";
		...
		if ($active == 1) {
			...
			$change = "title=\"Выкл.\"><img src=\"ad_min/pic/inactive.gif\" border=\"0\" alt=\"Выкл.\"></a>";
		} elseif ($active == 0) {
			$active = "<font color=\"#FF0000\">Выкл.</font>";
			$change = "title=\"Вкл.\"><img src=\"ad_min/pic/activate.gif\" border=\"0\" alt=\"Вкл.\"></a>";
		}
		...
		echo "<td align=\"center\"><a href=\"".$admin_file.".php?op=BlocksEdit&bid=$bid\" title=\"Редактировать\"><img src=\"ad_min/pic/edit.gif\" border=\"0\" alt=\"Редактировать\"></a> <a href=\"".$admin_file.".php?op=BlocksChange&bid=$bid\" $change";
		if ($bkey == "") echo " <a href=\"".$admin_file.".php?op=BlocksDelete&bid=$bid\" OnClick=\"return DelCheck(this, 'Удалить &quot;$title&quot;?');\" title=\"Удалить\"><img src=\"ad_min/pic/delete.gif\" border=\"0\" alt=\"Удалить\"></a>";
		if ($block_act == 0) echo " <a href=\"".$admin_file.".php?op=BlocksShow&bid=$bid\" title=\"Показать\"><img src=\"ad_min/pic/show.gif\" border=\"0\" alt=\"Показать\"></a>";
	}
...
function BlocksNew() {
	...
	while ($file = readdir($handle)) {
		if (preg_match("/^block\-(.+)\.php/", $file, $matches)) {
			...
			if (mysqli_num_rows(sql_query("SELECT * FROM ".$prefix."_blocks WHERE blockfile='$file'")) == 0) echo "<option value=\"$file\">$found</option>\n";
		}
	}
	closedir($handle);
	echo "</select></td></tr>"
	...
	$a = 1;
	$i = 0; // добавлена инициализация переменной $i
...
function BlocksAdd($title, $content, $bposition, $active, $hide, $blockfile, $view, $expire, $action) {
	global $prefix, $admin_file;
	list($weight) = mysqli_fetch_row(sql_query("SELECT weight FROM ".$prefix."_blocks WHERE bposition=".sqlesc($bposition)." ORDER BY weight DESC"));
	$weight++;
	$bkey = "";
	$btime = "";
	if ($blockfile != "") {
...
function BlocksEdit($bid) {
	...
	BlocksNavi();
	$bid = intval($bid);
	list($bkey, $title, $content, $bposition, $weight, $active,$hide, $blockfile, $view, $expire, $action, $which) = mysqli_fetch_row(sql_query("SELECT bkey, title, content, bposition, weight, active,allow_hide, blockfile, view, expire, action, which FROM ".$prefix."_blocks WHERE bid='$bid'"));
	if ($blockfile != "") {
		$type = "(Файловый блок)";
	} else {
		$type = "(HTML блок)";
...
function BlocksEditSave($newexpire, $bid, $bkey, $title, $content, $oldposition, $bposition, $active,$hide, $weight, $blockfile, $view, $expire, $action) {
	...
	if (isset($_POST['blockwhere'])) {
		...
		if ($oldposition != $bposition) {
			...
			$fweight = $weight;
			$oweight = $weight;
			while (list($nbid) = mysqli_fetch_row($result5)) {
				$weight++;
			...
			while (list($obid) = mysqli_fetch_row($result6)) {
				sql_query("UPDATE ".$prefix."_blocks SET weight=".sqlesc($oweight)." WHERE bid=".sqlesc($obid));
				$oweight++;
			}
			list($lastw) = mysqli_fetch_row(sql_query("SELECT weight FROM ".$prefix."_blocks WHERE bposition=".sqlesc($bposition)." ORDER BY weight DESC LIMIT 0,1"));
			if ($lastw <= $fweight) {
				$lastw++;
...
function BlocksShow($bid) {
	global $prefix, $db, $admin_file;
	BlocksNavi();
	list($bid, $bkey, $title, $content, $bposition, $blockfile) = mysqli_fetch_row(sql_query("SELECT bid, bkey, title, content, bposition, blockfile FROM ".$prefix."_blocks WHERE bid='$bid'"));
	$bid = intval($bid);
	echo "<p />";
...
function BlocksChange($bid, $ok=0) {
	global $prefix, $admin_file;
	$bid = intval($bid);
	$row = mysqli_fetch_array(sql_query("SELECT active FROM ".$prefix."_blocks WHERE bid='$bid'"));
	...
	if (($ok) || ($active == 0)) {
		...
		list($title, $content, $active) = mysqli_fetch_row(sql_query("SELECT title, content, active FROM ".$prefix."_blocks WHERE bid='$bid'"));
		if ($active == 0) {
...
switch($op) {
	...
	list($bposition, $weight) = mysqli_fetch_row(sql_query("SELECT bposition, weight FROM ".$prefix."_blocks WHERE bid='$bid'"));
