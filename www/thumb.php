...
$thumb_size = 160; //all thumbnails are this maximum width or height if not specified via get
$site_config['absolute_uri']=str_replace('///','//',str_replace('thumb.php?'.$_SERVER['QUERY_STRING'],'',$_SERVER['REQUEST_URI']));
$site_config['path_thumbnail']=$site_config['absolute_uri'].'/cache/thumbs/';	//where to cache thumbnails on the server, relative to the DOCUMENT_ROOT
$image_error=$site_config['absolute_uri'].'/pic/secret.jpg';	// used if no image could be found, or a gif image is specified
