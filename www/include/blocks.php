...
function render_blocks($side, $blockfile, $blocktitle, $content, $bid, $bposition, $allow_hide) {
  ...
  if ($allow_block_hide && ($allow_hide || get_user_class() >= UC_ADMINISTRATOR)) {
    $hidden_blocks = (isset($_COOKIE['hb']) && $_COOKIE['hb'] ? unserialize($_COOKIE['hb']) : array());
    $display = 'block';
    $picture = 'minus';
    $alt = 'Скрыть';
    ...
    if ($bl_mass[$bl_name]['m']) {
        ...
        if (!isset($bl_mass[$bl_name])) {
            if (file_exists("themes/" . $ss_uri . "/html/" . $bl_name . ".html")) {
                ...
                $f_str = 'global $ss_uri, $lang; ' . $func . ' "' . addslashes($f_str) . "\"" . $func2 . ";";
                $bl_mass[$bl_name]['f'] = function ($title, $content) use ($f_str) {
                    eval($f_str);
                };
            } else {
                $bl_mass[$bl_name]['m'] = false;
...
function show_blocks($position) {
    global $CURUSER, $use_blocks, $already_used, $orbital_blocks, $DEFAULTBASEURL, $mysqli;
    static $showed_show_hide;

    if ($use_blocks) {
        if (!$already_used) {
            $read = readCache("blocks.cache", "180000");
            if ($read == FALSE) {
                ...
                $text = serialize($orbital_blocks);
                writeCache($text, "blocks.cache");
            } else {
                $orbital_blocks = unserialize($read);
            }
        ...
        foreach ($orbital_blocks as $block) {
            if (!$showed_show_hide)
                echo '<script language="javascript" type="text/javascript" src="' . $DEFAULTBASEURL . '/js/blocks.js"></script>';
          ...
          if ($view == 0) {
    render_blocks(isset($side) ? $side : 'left', $blockfile, $title, $content, $bid, $bposition, $allow_hide);
