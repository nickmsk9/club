...
class smtp
{
	...
	*/
    ...
    /**
     * Checks if the server has closed the connection or denied commands.
     *
     * @param string|false $response Server response text or false if none
     * @return bool True if connection is closed
     */
    private function _is_closed(string|false $response = false): bool
    {
        if (!$this->_connection) {
            return true;
        }
        if (is_string($response) && ($response[0] === '4' || $response[0] === '5')) {
            $this->close();
            return true;
        }
        return false;
	...
	*/
    ...
    /**
     * Perform SMTP authentication using various mechanisms.
     *
     * @param string $username SMTP username
     * @param string $password SMTP password
     * @param int    $type     Authentication mechanism (PLAIN, LOGIN, CRAM_MD5)
     * @return void
     */
    public function auth(string $username, string $password, int $type = LOGIN): void
    {
        global $rootpath;
