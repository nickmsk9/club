<?php
require_once 'include/bittorrent.php';

dbconn();
include_once 'include/Wall_Updates.php';
$Wall = new Wall_Updates();
$invalid = "";
$xyz="       ";
                  //iVBORw0KGgoAAAANSUhEUgAAAUAAAADwCAYAAABxLb1rAAAgAElEQVR4nHR913bb2Lbs%2FsM7xj1ntxWZg0gxEwADwADmnMQcAGZSycpZsrv7nP%2Bq%2B7CwFu3e4z7MYYFikG2oUHNWzcK%2F%2Fl7W8T%2BrBv53fYa%2Fl0382DTxvqjix7yGz2kF78s8PlYFvC%2FzeF%2Fm8TnL4XOWww8li69JBj8mZXwMC3gblvA2LOF1UMTroIiPYQbvgzQ%2BJgV8TAp4VzN4Gct4H6Tx1k%2FhZZD5rR7HcTxNEngbJPHaT%2BCpl8JzP42nvoTnQQwfvQTeu3G8tRN4acVw15Fx15Hx3BZJdWQ8tOJ4bqTw3Ejho5XFRyuL%2B46I27MIbmphXFcEfG%2BG8L0Zwk0tjNt6BE%2FVCF7qIr43YriuS3iqxvBUjeG2HsF1RcBlVcB1PYzrKoebuoDrehSX1TCuKkFcVzlsa3Fsa3FcFRO4LMRxlw%2FjLh%2FGZVHCZVHCtsJhUw5iWfNjVQ
if ($_POST['type'] == "pixel") 
{
	$image = $_POST['image'];
	$filter_image = str_replace("data:image/png;base64,", "", $image);
    // input is in format 1,2,3...|1,2,3...|...
	if($filter_image == $invalid)
		{
			$im = "";
			echo "false";
		}
	else
	{
		$im = imagecreatetruecolor(320, 240);
		foreach (explode("|", $_POST['image']) as $y => $csv) {
			foreach (explode(";", $csv) as $x => $color) {
				imagesetpixel($im, $x, $y, $color);
			}
		}
	}
} else {
    // input is in format: data:image/png;base64,...
	$image = $_POST['image'];
	$filter_image = str_replace("data:image/png;base64,", "", $image);
	if($filter_image == $invalid)
		{
			$im = "";
			echo "false";
		}
	else
		$im = imagecreatefrompng($_POST['image']);
}
if($im)
	{
$filename=time().$CURUSER["id"].".jpg";


imagejpeg($im);
imagejpeg($im, $_SERVER['DOCUMENT_ROOT']."/uploads/".$filename);
$data=$Wall->Image_Upload($CURUSER["id"],$filename);
echo $_SERVER['DOCUMENT_ROOT']."/uploads/".$filename;
} else {
die('error');
}
?>
